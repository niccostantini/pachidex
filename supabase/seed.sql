...
insert into items (nome, categoria, rarita, croquembouche, ripetibile, validazione, note, lat, lng, riferimento) values ('La rivincita', 'attivita', 'leggendario', 60, false, 'foto', 'Portare a termine la cosa che l''anno scorso non ti è riuscita. Sai già qual è.', null, null, null) on conflict do nothing;
...
begin
	...
	join users u on u.id = c.user_id
	where u.nome = 'Ciccio' order by c.timestamp desc limit 1;
	select id into v_chi from users where nome = 'Rosa';
